...
class SubscriptionController extends Controller
{
    ...
    public function showSubscriptionOptions($username)
    {
        ...
        $systemPrices = SubscriptionPriceSystem::all();
        $existingSubscription = SubscriptionUser::where('user_id', Auth::id())
            ->where('target_user_id', $user->id)
            ->where('end_date', '>', Carbon::now())
            ->first();
        $existingComboSubscription = SubscriptionCombo::where('user_id', Auth::id())
            ->where('target_user_id', $user->id)
            ->where('end_date', '>', Carbon::now())
            ->first();
    ...
    public function checkTransactionStatusUser(Request $request)
    {
        ...
        try {
            ...
            Log::error('Midtrans Error CheckTransactionsUser: ' . $e->getMessage());
            return response()->json(['status' => 'error', 'message' => 'Could not connect to Midtrans. Please try again later.'], 500);
        }
    }
    ...
    public function subscribeCombo(Request $request, $username)
    {
        ...
        try {
            ...
            Log::error('Midtrans Error SubscribeCombo: ' . $e->getMessage());
            return response()->json(['error' => 'Could not connect to Midtrans. Please try again later.'], 500);
        }
    }
    ...
    public function checkTransactionStatusCombo(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($status, &$redirectUrl) {
                ...
                if ($status->transaction_status == 'settlement') {
                    $userPrice = SubscriptionPriceUser::where('user_id', $transaction->target_user_id)->firstOrFail();
                    $systemPrices = SubscriptionPriceSystem::all();
    
                    $found = false;
                    foreach ($systemPrices as $systemPrice) {
                        if ($grossAmount == $userPrice->price_1_month + $systemPrice->price ||
                            $grossAmount == $userPrice->price_3_months + $systemPrice->price ||
                            $grossAmount == $userPrice->price_6_months + $systemPrice->price ||
                            $grossAmount == $userPrice->price_1_year + $systemPrice->price) {
                            $found = true;
                            break;
                        }
                    }
                    ...
                    Log::info('User Price found:', $userPrice->toArray());
                    Log::info('System Price found:', $systemPrice->toArray());
    
                    $duration = $this->getDurationFromPrice($userPrice, $grossAmount - $systemPrice->price);
                    $startDate = Carbon::now();
                    $endDate = $startDate->copy()->addMonths($duration);
                    ...
                    // Update or create combo subscription
                    $existingSubscriptionCombo = SubscriptionCombo::where('user_id', $user->id)
                        ->where('target_user_id', $targetUser->id)
                        ->where('end_date', '>', Carbon::now())
                        ->first();
    
                    if ($existingSubscriptionCombo) {
                        $existingSubscriptionCombo->update([
                            'system_price' => $systemPrice->price,
                            'user_price' => $grossAmount - $systemPrice->price,
                            'total_price' => $grossAmount,
                            'end_date' => $endDate,
                            'transaction_id' => $transaction->id,
                        ]);
                        Log::info('Existing combo subscription updated:', $existingSubscriptionCombo->toArray());
                    } else {
                        $newSubscriptionCombo = SubscriptionCombo::create([
                            'user_id' => $user->id,
                            'target_user_id' => $targetUser->id,
                            'system_price' => $systemPrice->price,
                            'user_price' => $grossAmount - $systemPrice->price,
                        ...
                        Log::info('New combo subscription created:', $newSubscriptionCombo->toArray());
                    }
                    ...
                    $notif = Notif::create([
                        ...
                        'target_id' => $transaction->id,
                        'type' => 'system',
                        'message' => 'Selamat! Anda telah berlangganan ke '. $targetUser->username .' dan sistem sampai dengan ' . $endDate->format('d F Y'),
                    ]);
            ...
            Log::error('Midtrans Error CheckTransactionsCombo: ' . $e->getMessage());
            return response()->json(['status' => 'error', 'message' => 'Could not connect to Midtrans. Please try again later.'], 500);
        }
    }
